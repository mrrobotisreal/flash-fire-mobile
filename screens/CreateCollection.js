...
import { View, Text, TextInput, Pressable, StyleSheet, FlatList, Dimensions, Image, ImageBackground, StatusBar, TouchableOpacity, Modal, ToastAndroid, ScrollView } from 'react-native';
...
export default function CreateCollection({ navigation }) {
  const { storeCollection } = useContext(DbContext);
  ...
  const validateCollection = () => {
    if (!name) {
      ...
      ToastAndroid.show('Please add at least 1 card, or 6+ cards for optimal testing', ToastAndroid.SHORT);
      return false;
    } else {
      return true;
    }
          ...
          <View style={styles.collectionsContainer}>
            ...
            <View style={styles.innerContainer}>
              ...
              <View style={styles.audioButtonsContainer}>
                ...
                >
                  <MaterialCommunityIcons style={styles.recordAudioIcon} name="record-rec" size={24} color={isRecording ? 'red' : 'white'} />
                </Pressable>
                <Pressable
                  onPress={isPlaying ? StopPlaying : PlayRecordedAudio}
                  style={{
                    ...styles.recordAudioButton,
                    borderColor: recordedUri ? isPlaying ? 'green' : 'red' : 'grey',
                  }}
                >
                  <MaterialCommunityIcons style={styles.recordAudioIcon} name="play" size={24} color={recordedUri ? isPlaying ? 'green' : 'white' : 'grey'} />
                </Pressable>
                {recording
                  ? (
                    <Text style={styles.itemDescription}>
